...
var app = {
   ...
   onParseAdvData: function(device){
        ...
        if (advertisement.localName == 'heartworm' &&
			...
			if (mandata[0] == 224) {
				//app.log(mandata);
				if(mandata[3]) {
        			  document.getElementById("daysVal").innerHTML = mandata[3].toString();
				}
				if(mandata[4]) {
        			  document.getElementById("hoursVal").innerHTML = mandata[4].toString();
				}
				if(mandata[5]) {
        			  document.getElementById("minsVal").innerHTML = mandata[5].toString();
				}
			}
